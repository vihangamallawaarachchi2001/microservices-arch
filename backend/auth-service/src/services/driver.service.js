...
export const register = async (registrationData) => {
  try {
    ...
    const existingUser = await Driver.findOne({ email: registrationData.email });
...
export const activateAccount = async (activationData, req) => {
  try {
    ...
    const driver = await Driver.findOne({ email: activationData.email });
    if (!driver) throw new ApiError(404, 'driver not found');
    driver.isActive = true;
    await driver.save();

    const token = await generateToken({ id: driver._id, email: driver.email });
...
export const login = async (emailOrUsername, password, req) => {
  try {
    ...
    if (!isPasswordValid) return { success: false, message: 'Invalid drivername or password' };
    if (!driver.isActive) return { success: false, message: 'Please activate your account before logging in' };
    ...
    console.log(1);
    const token = await generateToken({ userId: driver._id });
